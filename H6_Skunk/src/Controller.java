...
public class Controller
{
		...
		while (gameNotOver)
		{
			...
			while (wantsToRoll)
			{
				...
				else if (isSkunkDeuce())
				{
					...
					kitty += 2;
					activePlayer.scoreSkunkRoll(SKUNK_DEUCE_CHIP_PENALTY);
					wantsToRoll = false;
					break;
				}
	...
	// Third refactor using method extraction
	private boolean isDoubleSkunk() {
		return skunkDice.getLastRoll() == 2;
	}
	
	// Second refactor using method extraction
	private boolean isRegularSkunk() {
		return skunkDice.getDie1().getLastRoll() == 1 || skunkDice.getDie2().getLastRoll() == 1;
	}
	
	// First refactor using method extraction to reduce repetitive 
	// lines of code from prompting the user if they would like to keep rolling.
